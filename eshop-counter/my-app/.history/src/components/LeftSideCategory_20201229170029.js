...
const LeftSideCategory = () => {
    ...
    let categoryComponents = CategoriesSelect.map(categObject => <li className='hover:bg-gray-400 pl-5 h-12 text-2xl cursor-pointer leading-loose font-bold' key={categObject.id}>{categObject.name}</li>)
    return(
        <div  className='leftSide w-1/4 '>
            <ul>
                <li className='hover:bg-gray-400 pl-5 h-12 text-2xl cursor-pointer leading-loose font-bold'><div>All</div></li>
                {categoryComponents}
            </ul>
        </div>
    )

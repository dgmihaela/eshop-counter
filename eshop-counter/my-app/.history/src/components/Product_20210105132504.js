...
const Product = ({callback, product}) => {
   ...
   let totalCostGlobal = constNewProduct.totalCost;
    ...
    const setIncrement =  dispatch(setQuantity({productId: product.id, quantity: thisCount > 1 ? thisCount - 1 : 1 }));
    function decrementAction() {
      
        Promise(setIncrement).then(() => {
            counterAction();
        });
    }


    const setDecrement = dispatch(setQuantity({productId: product.id, quantity: thisCount + 1 })).then(counterAction());
    function incrementAction() {
        
        Promise(setDecrement).then(() => {
            counterAction();
        });
    }

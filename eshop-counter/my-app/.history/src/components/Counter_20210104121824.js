...
const Counter = ({product}) => {
    ...
    function incrementComp(event) {
        ...
        dispatch(setQuantity({productId: product.id, nrItems: countItems.nrItems }))
    }
    ...
    return(
        <div>
            <div className='btns-counter h-12 box-border flex flex-wrap content-start'>
                <button onClick={(e) => decrementComp(e)} className='inline-block rounded rounded-r-none w-12 h-12 bg-gray-200 pl-3 pt-1.5 hover:bg-gray-300' >
                    <div className='w-6 h-6 bg-no-repeat bg-contain' style={{backgroundImage: `url(${minus})`}}></div>
                </button>
                <p className='inline-block w-12 h-12 text-center bg-gray-100 leading-10 '><span>{countItems.nrItems + 1}</span></p>
                <button onClick={(e) => incrementComp(e)} className='inline-block rounded rounded-l-none w-12 h-12 bg-gray-200 pl-3 pt-1.5 hover:bg-gray-300' >

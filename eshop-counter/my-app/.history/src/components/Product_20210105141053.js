...
const Product = ({callback, product}) => {
   ...
   let totalCostGlobal = constNewProduct.totalCost;
    ...
    const decrementAction = (dispatch) => new Promise((resolve, reject) => {
      ...
      function scrolltoNextItem() {
        return (dispatch, counterAction) => {
            decrementAction( dispatch).then(() => {
            dispatch(
              scrollToNextIndex(
                counterAction()
              )
      ...
      const incrementAction = (dispatch) => new Promise((resolve, reject) => {
        ...
        <div className='w-80 h-96 bg-gray-50 shadow-md py-5 px-5 '>
            ...
            <div>
                <div className='btns-counter h-12 box-border flex flex-wrap content-start'>
                    <button type='button' onClick={scrolltoNextItem} className='inline-block rounded rounded-r-none w-12 h-12 bg-gray-200 pl-3 pt-1.5 hover:bg-gray-300' >

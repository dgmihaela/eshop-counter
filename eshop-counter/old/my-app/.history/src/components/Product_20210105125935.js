...
const Product = ({callback, product}) => {
   ...
   const newProduct = JSON.parse(JSON.stringify(product));
        ...
        const fetchData = async() => {
            ...
            const result = await axios.put(`http://localhost:9000/products/${product.id}`,
             {  title: product.title,
                ...
                stars: product.stars,
                id: product.id,
                quantity: thisCount.description,
    ...
    function counterAction() {
        ...
        if(thisCount === 1) {
            newProduct.totalCost = newProduct.price;
        } else if(thisCount > 1 ) {
            const priceIncreased = newProduct.price * thisCount
            newProduct.totalCost = (Math.round(priceIncreased * 100) / 100).toFixed(2);
           console.log(newProduct.totalCost)
        }
    }

    function decrementAction() {
        dispatch(updateQuantity({productId: product.id, quantity: thisCount > 1 ? thisCount - 1 : 1 }));
    ...
    function incrementAction() {
        dispatch(updateQuantity({productId: product.id, quantity: thisCount + 1 }));
        counterAction()
    }
    ...
    return(
        <div className='w-80 h-96 bg-gray-50 shadow-md py-5 px-5 '>
            ...
            <div className='btns-counter h-12 box-border flex flex-wrap content-start'>
                <button type='button' onClick={incrementAction} className='inline-block rounded rounded-r-none w-12 h-12 bg-gray-200 pl-3 pt-1.5 hover:bg-gray-300' >
                ...
                <button type='button' onClick={decrementAction} className='inline-block rounded rounded-l-none w-12 h-12 bg-gray-200 pl-3 pt-1.5 hover:bg-gray-300' >

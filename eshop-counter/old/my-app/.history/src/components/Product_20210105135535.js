...
const Product = ({callback, product}) => {
   ...
   let totalCostGlobal = constNewProduct.totalCost;
    ...
    function decrementAction() {
       dispatch(setQuantity({productId: product.id, quantity: thisCount > 1 ? thisCount - 1 : 1 })).then(counterAction)
       
    }

    function incrementAction() {
        dispatch(setQuantity({productId: product.id, quantity: thisCount + 1 })).then(counterAction());

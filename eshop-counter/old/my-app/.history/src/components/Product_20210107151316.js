...
const Product = ({selectedProduct, product }) => {
    ...
    const getProdCart = async () => {
        try {
           ...
           if(product.id){
            setIsInCart({productId:product.id, status: true })
           }
    ...
    const putProdCart = async () => {
        if(isPost === 1) {
            console.log('din post==================',thisCount)
            try {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const result = await putProdCart();
        }
        fetchData();
    }, [thisCount]);
console.log('thisCount dupa 2',thisCount)


    function counterAction(newCount) {
        let totalCostGlobal = constNewProduct.price;
        console.log('counter action: thisCount = ' + newCount);

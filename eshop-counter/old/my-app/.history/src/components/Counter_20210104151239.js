...
const Counter = ({product}) => {
    ...
    const [counter, setCounter] = useState(thisCount);
    ...
    return(
        ...
        <div>
            <div className='btns-counter h-12 box-border flex flex-wrap content-start'>
                <button type='button' onClick={() =>{setCounter(thisCount); dispatch(setQuantity({productId: product.id, quantity: thisCount > 0 ? thisCount - 1 : 0 }))}} className='inline-block rounded rounded-r-none w-12 h-12 bg-gray-200 pl-3 pt-1.5 hover:bg-gray-300' >

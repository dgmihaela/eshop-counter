   ...
   const constNewProduct = JSON.parse(JSON.stringify(product));
    ...
    useEffect(() => {
        const fetchData = async() => {
            ...
            const result = await axios.put(`http://localhost:9000/products/${constNewProduct.id}`,
             {  title: constNewProduct.title,
                description:constNewProduct.description,
                id_category: constNewProduct.id_category,
                price: constNewProduct.price,
                stars: constNewProduct.stars,
                id: constNewProduct.id,
                quantity: thisCount,
                totalCost: constNewProduct.totalCost
            });
    ...
    function counterAction() {
        ...
        console.log(constNewProduct.totalCost)

       if(thisCount === 1) {
        constNewProduct.totalCost = constNewProduct.price;
       } else if(thisCount > 1 ) {
        constNewProduct.totalCost = constNewProduct.price * thisCount;
       }
    }

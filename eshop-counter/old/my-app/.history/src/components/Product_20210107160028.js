...
const Product = ({ product }) => {
    ...
    const [isPost, setIsPost] = useState({isPost: false, prodId: product.id})
    ...
    const postProdCart = async () => {
        if(isPost.isPost === false && thisCount === 1) {
            console.log(isPost)
            try {
                const result = await axios.post(`http://localhost:9000/cart`,
                    ...
                    console.log(result)
                    setIsPost({...isPost,isPost: result.data.isPost, prodId: result.data.id})
    ...
    console.log('setul de dupa post', isPost)

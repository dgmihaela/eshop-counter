...
const LeftSideCategory = () => {
    ...
    let categoryComponents = CategoriesSelect.map(categObject => <li className='hover:bg-gray-400 pl-5 h-12 text-2xl' key={categObject.id}>{categObject.name}</li>)
    return(
        <div  className='leftSide bg-gray-100 w-1/3 '>
            <ul className=''>
                <li className='hover:bg-gray-400 pl-5 h-12 text-2xl'><div>All</div></li>
                {categoryComponents}
            </ul>
        </div>
    )

...
const Product = ({ product }) => {
    ...
    const [isPost, setIsPost] = useState({isPost: ''})
        ...
        if(isPost === false && thisCount === 1) {
            try {
                const result = await axios.post(`http://localhost:9000/cart`,
                    ...
                    console.log(result.data.isPost)
                    setIsPost({...isPost, isPost: result.data.isPost})
    ...
    console.log('setul isPost', isPost)
